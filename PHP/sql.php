...
//Load .env From This Dir And 3 Levels Up
LoadENV("../../../.env");
LoadENV("../../.env");
LoadENV("../.env");
LoadENV("./.env");
...
DB::$dbName = getenv("DB_ANALYTICS");
DB::$host = getenv("DB_HOST");

if (getenv("DB_ANALYTICS") == false) {
    http_response_code(500);
    die('[{"type":"error","error":"Database Credentials Were Not Found In Environment"}]');
}
if (!function_exists("mysqli_connect")) {
    http_response_code(500);
    die('[{"type":"error","error":"MYSQL Is Not Available"}]');
}
if (count(DB::query("SHOW TABLES LIKE 'config'")) != 1 && !isset($setupMode)) {
