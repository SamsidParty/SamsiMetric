/* DISABLE LINE WRAPPING TO VIEW SOURCE */

//localForage Version 1.10.0 (Loader Dependency)
...
//Some Scripts Are Needed For Installation But Not Needed Later
...
var devMode = latestVersion.startsWith("dev");

//The JS Files That Will Be Executed On This Page (In Chronological Order)
//Seperated By A Backslash
var initalDependencies = document.querySelector('meta[name="dependencies"]').content
.replaceAll("\n", "").replaceAll("\t", "").replaceAll("\r", "").replaceAll(" ", "").split("\\");
...
async function MainLoader() {
    ...
    for (var l_dep of initalDependencies) {
        if (l_dep.length > 0) { await LoadDependency(l_dep.replace("//", "./JS/")); }
    }
...
//Install Or Update To The Latest Client Version
...
async function InstallDynamic() {
    console.log(`Dynamic Install Started (Version ${latestVersion})`);
    ...
    for (var l_dep of buildDependencies) {
        var dep = await (await fetch(l_dep)).text();
        (1, eval)(dep);
    }
    ...
    for (var l_dep of collectedDependencies) {
        var file = await (await fetch(l_dep)).blob();
        var script = await file.text();
        console.log(`Downloaded ${l_dep}`);

        if (script.includes("AutoLoadThisFile();") && !l_dep.includes("/loader.dev.js") && !l_dep.includes("/loader.prod.js")) {
            installerContext.autoload.push(l_dep);
        }
    ...
    localforage.setItem("./bundle.css", cssBundle);

    localforage.setItem("client-version", latestVersion);
    localforage.setItem("installer-context", JSON.stringify(installerContext));
    console.log(`Install Finished (Version ${latestVersion})`);
}

//Fetch Indexes To Get All Needed Files
async function CollectDependencies(installerContext) {

    var collectedDependencies = [];

    //Collect CSS Files
    var cssFiles = ParseIndexTree(await (await fetch("./CSS/")).text(), (l_file) => { return l_file.endsWith(".css"); });
    cssFiles.forEach((l_file) => collectedDependencies.push("./CSS" + l_file));

    //Collect JS Files
    var jsFiles = ParseIndexTree(await (await fetch("./JS/")).text(), (l_file) => { return l_file.includes(".js"); });
    jsFiles.forEach((l_file) => collectedDependencies.push("./JS" + l_file));

    //Collect Font Files
    var fontFiles = ParseIndexTree(await (await fetch("./Fonts/")).text(), (l_file) => { return l_file.includes(".ttf") || l_file.includes(".woff"); });
...
async function LoadDependency(dependency) {
    ...
    for (var l_listener of loadListeners) {
        if (l_listener.for == dependency) {
            await l_listener.run();
        }
    }
...
async function CreateStaticBundle(version) {
    ...
    if (version) {
        await localforage.setItem("client-version", version);
    }
    ...
    var bundle = ""; // FORMAT: <script name> <seperation char> <script content> <seperation char> etc..

    await localforage.iterate((script, l_dep) => {
        //Ignore Cache
        if (!l_dep.startsWith("IconCache")) {
            bundle += l_dep;
            bundle += String.fromCharCode(0x1C);
            bundle += script;
            bundle += String.fromCharCode(0x1C);
        }
    });
...
async function StartReact() {
    ...
    var initFunction = `window.root = ReactDOM.createRoot(document.getElementById("root")); root.render(React.createElement(${document.body.getAttribute("method")}));`;
    (1, eval)(initFunction)
}

function RunOnLoad(dependency, callback) {
...
async function LoadBinaryDependency(dep, value) {

    if (loadedBinaryDependencies[atob(dep)] != undefined) { return; } // Already Loaded
...
async function Loader_WaitUntil(cond) {
    if (cond()) { return; }
    const delayMs = 10;
    while(!cond()) await new Promise(resolve => setTimeout(resolve, delayMs));
}

function AutoLoadThisFile() {} // Dummy Function

function LoadingScreen() {

    if (!document.body) { addEventListener("load", LoadingScreen); return; }
    ...
    //Change Page Based On devMode
    document.body.innerHTML += `
    
        <div class="loaderPage">
            ${ devMode ? '<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-code-dots" width="240" height="240" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M15 12h.01" /><path d="M12 12h.01" /><path d="M9 12h.01" /><path d="M6 19a2 2 0 0 1 -2 -2v-4l-1 -1l1 -1v-4a2 2 0 0 1 2 -2" /><path d="M18 19a2 2 0 0 0 2 -2v-4l1 -1l-1 -1v-4a2 2 0 0 0 -2 -2" /></svg>' : `<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-world-download" width="240" height="240" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M21 12a9 9 0 1 0 -9 9" /><path d="M3.6 9h16.8" /><path d="M3.6 15h8.4" /><path d="M11.578 3a17 17 0 0 0 0 18" /><path d="M12.5 3c1.719 2.755 2.5 5.876 2.5 9" /><path d="M18 14v7m-3 -3l3 3l3 -3" /></svg>` }
            <h1>${devMode ? "Recompiling Scripts" : "Updating - This May Take A While The First Time"}</h1>
        </div>
